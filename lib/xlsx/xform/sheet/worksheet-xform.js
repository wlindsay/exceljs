  ...
  prepare(model, options) {
    ...
    model.media.forEach(medium => {
      if (medium.type === 'background') {
        ...
        let {drawing} = model;
        bookImage = options.media[medium.chartId];
        if (!drawing) {
          drawing = model.drawing = {
            rId: nextRid(rels),
            name: `drawing${++options.drawingsCount}`,
        ...
        let rIdImage =
          this.preImageId === medium.chartId
            ? drawingRelsHash[medium.chartId]
            : drawingRelsHash[drawing.rels.length];
        if (!rIdImage) {
          rIdImage = nextRid(drawing.rels);
          drawingRelsHash[drawing.rels.length] = rIdImage;
          drawing.rels.push({
            Id: rIdImage,
            Type: 'http://schemas.openxmlformats.org/drawingml/2006/chart',
            Target: `../charts/chart${medium.chartId}.xml`,
          });
          drawing.rels.push({
            Id: nextRid(drawing.rels),
            Type: 'http://schemas.microsoft.com/office/drawing/2012/chartStyle',
            Target: `../charts/colors${medium.chartId}.xml`,
          });
          drawing.rels.push({
            Id: nextRid(drawing.rels),
            Type: 'http://schemas.microsoft.com/office/drawing/2012/chartStyle',
            Target: `../charts/style${medium.chartId}.xml`,
          });
        }
