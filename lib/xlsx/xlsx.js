  ...
  async addMedia(zip, model) {
    await Promise.all(
      model.media.map(async medium => {
        ...
        } else if (medium.type === 'chart') {
          zip.append(medium.chart, {name: `xl/charts/chart${medium.chartId}.xml` });
          zip.append(medium.colors, {name: `xl/charts/colors${medium.chartId}.xml` });
          zip.append(medium.style, {name: `xl/charts/style${medium.chartId}.xml` });

          const relsXform = new RelationshipsXform();
          const xml = relsXform.toXml([
            { Id: 'rId2', Type: 'http://schemas.microsoft.com/office/2011/relationships/chartColorStyle', Target: `colors${medium.chartId}.xml` },
            { Id: 'rId1', Type: 'http://schemas.microsoft.com/office/2011/relationships/chartStyle', Target: `style${medium.chartId}.xml` }
          ]);
          zip.append(xml, {name: `xl/charts/_rels/chart${medium.chartId}.xml.rels`});
          return;
        }
